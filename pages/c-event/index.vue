<template>
  <div class="event-page content-wrap c-event-page">
    <div class="subpage-header event-subpage-header">
      <NuxtLink :to="'/'+$i18n.locale+'/c-congress'" class='router-back'>
    ...
    <section class="event-section flex space-btw">
      ...
      <div class="event-section-item event-title-item">
        ...
        <Button v-if="(this.$route.query.type === 'recent')"
                v-html="$t('event.eventBtn')" class="btn-vue__ligth hide-mobile"
                @click="linkToPage(event.attributes?.facebook_link)">
        </Button>
        <Button v-if="(this.$route.query.type === 'past')"
                v-html="$t('event.eventBtnPast')" class="btn-vue__ligth hide-mobile"
                @click="linkToPage(event.attributes?.facebook_link)">
        </Button>
      </div>
    </section>
...
export default {
  ...
  data () {
    return {
      backBtnText:'Повернутись до усіх подій',
      btnText:'Відвідати',
      event: {}
    }
  },
  ...
  mounted () {
    ...
    if (this.$route.query.type === 'recent') {
      eventUrl = '/api/v1/congress_centers/recent'
    }
    if (this.$route.query.type === 'past') {
      eventUrl = '/api/v1/congress_centers/past'
    }
